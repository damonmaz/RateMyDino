...
export default function ProfessorPage() {
	...
	const profName = decodeURIComponent(rawProfName)
		.replace(/[^a-zA-Z\s]/g, "")
		.trim();
		...
		const fetchProfessorData = async () => {
			try {
				...
				const [
					...
					fetch(
						`https://ratemydino.onrender.com/professor/${encodeURIComponent(
							profName
						)}/summary`
					),
					fetch(
				...
				const uniqueCourses = Array.isArray(coursesData.Courses)
					? [...new Set(coursesData.Courses)]
					: [];
							...
							<>
								<div
									className={`transition-all duration-700 ${
										fadeInStep >= 1 ? "opacity-100" : "opacity-0"
									}`}
								>
								...
								<div
									className={`flex flex-wrap gap-2 mt-4 transition-all duration-700 ${
										fadeInStep >= 2 ? "opacity-100" : "opacity-0"
									}`}
								>
								...
								<div
									className={`mt-4 transition-all duration-700 ${
										fadeInStep >= 3 ? "opacity-100" : "opacity-0"
									}`}
								>
								...
								<div
									className={`mt-6 bg-gray-100 p-6 rounded-lg shadow transition-all duration-700 ${
										fadeInStep >= 4 ? "opacity-100" : "opacity-0"
									}`}
								>
								...
								<div
									className={`mt-6 border-b pb-4 flex flex-col sm:flex-row gap-2 transition-all duration-700 ${
										fadeInStep >= 4 ? "opacity-100" : "opacity-0"
									}`}
								>
									<input
								...
								<div className="mt-6 space-y-4">
									{Array.isArray(professorCourses) &&
